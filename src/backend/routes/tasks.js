...
function buildListQuery({ userId, search, status, priority, category }) {
...
function getSingleRow(dbConn, sql, params) {
...
function getAllRows(dbConn, sql, params) {
  return new Promise((resolve, reject) => {
...
// GET /api/tasks - Get all tasks for logged-in user (search + filters + pagination)
router.get('/', async (req, res) => {
  ...
  if (limit === null || limit > MAX_LIMIT || (!ALLOWED_LIMITS.has(limit) && limit !== DEFAULT_LIMIT)) {
    return res.status(400).json({
      error: 'BAD_REQUEST',
      message: `Invalid 'limit' parameter. Allowed values: ${[...ALLOWED_LIMITS].join(', ')} (max ${MAX_LIMIT}).`,
    });
  }
  ...
  try {
    const countRow = await getSingleRow(
      db,
