...
router.get('/', (req, res) => {
    try {
        ...
        const sortField = typeof sortBy === 'string' && validSortFields.has(sortBy) ? sortBy : 'created_at';
        const sortOrder = typeof order === 'string' && ['asc', 'desc'].includes(order.toLowerCase()) ? order.toLowerCase() : 'desc';
        if (typeof sortBy !== 'undefined' && (typeof sortBy !== 'string' || !validSortFields.has(ortBy)) {
        ...
        let orderByClause = `| created_at ${sortOrder.toUpperCase()}, id ASC`;
        ...
        } else if (['due_date', 'created_at', 'title'].includes(sortField)) {
            orderByClause = `${sortField} ${sortOrder.toUpperCase()}, id ASC`;
        }
